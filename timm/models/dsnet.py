...
__all__ = [
    'ds_net_tiny', 'ds_net_small', 'ds_net_large'
]
...
class TMixBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, downsample=2, conv_ffn=False):
        super().__init__()
        ...
        self.norm_sa1 = nn.LayerNorm(self.dim)
        self.channel_up = nn.Linear(self.dim, 3 * self.dim)
        self.attn = Attention(
            self.dim,
        ...
        if conv_ffn:
            self.mlp = DWConvMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        else:
    ...
    def forward(self, x):
        ...
        sa = x.flatten(2).transpose(1, 2) #拉平
        residual_sa = sa
        sa = self.norm_sa1(sa)  # norm_sa1是 LayerNorm，channel方向做归一化，算CHW的均值,主要对RNN作用明显；
        sa = self.channel_up(sa) #  nn.Linear 输出3dim_sa
...
class MixBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # qkv = x[:, :(self.dim // 2), :]
        # conv = x[:, (self.dim // 2):, :, :]
        qkv = x[:, :self.dim_sa, :]  # channel纬度切分
        conv = x[:, self.dim_sa:, :, :]
        residual_conv = conv
        conv = residual_conv + self.conv(self.norm_conv1(conv))  # 原卷积 + 归一化卷积; BatchNorm：batch方向做归一化，算NHW的均值

        # sa = self.attn_down(qkv)
        sa = nn.functional.interpolate(qkv, size=(H // self.downsample, W // self.downsample), mode='bilinear') # 进行除2的下采样
        B, _, H_down, W_down = sa.shape
        sa = sa.flatten(2).transpose(1, 2) #拉平
        residual_sa = sa
        sa = self.norm_sa1(sa)  # norm_sa1是 LayerNorm，channel方向做归一化，算CHW的均值,主要对RNN作用明显；
        sa = self.channel_up(sa) #  nn.Linear 输出3dim_sa
        ...
        residual_sa_co = sa
        conv_qkv = self.cross_channel_up_conv(self.norm_conv2(conv)) #  对conv做nn.Linear 输出3dim_cov
        conv_qkv = conv_qkv.flatten(2).transpose(1, 2) #拉平
        #print('sa:', self.norm_sa2(sa).shape)
        #input()
        sa_qkv = self.cross_channel_up_sa(self.norm_sa2(sa))  # norm_sa2是 LayerNorm，cross_channel_up_sa 做nn.Linear 输出3dim_sa
        ...
        # sa -> conv
        conv = self.cross_attn(conv_q, sa_k, sa_v, shape=(B_conv, N_conv, C_conv)) # 用sa_k, sa_v 对conv_q 做cross attn
        conv = self.fuse_channel_conv(conv) # fuse_channel_conv 做个nn.Linear, dim_conv -> dim_conv
        conv = conv.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        conv = residual_conv_co + conv  # 加没有做cross前的conv
        #print('conv:', conv.shape)
        #input()
        # conv -> sa
        sa = self.cross_attn(sa_q, conv_k, conv_v, shape = (B_sa, N_sa, C_sa)) # 用 conv_k, conv_v 对sa_q 做cross attn
...
class MixVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 representation_size=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=None,
                 overlap_embed=False, conv_ffn=False):
        """
        Args:
        ...
        if overlap_embed:
            self.patch_embed1 = OverlapPatchEmbed(
                img_size=img_size, patch_size=7, stride=4, in_chans=in_chans, embed_dim=embed_dim[0])
            self.patch_embed2 = OverlapPatchEmbed(
                img_size=img_size // 4, patch_size=3, stride=2, in_chans=embed_dim[0], embed_dim=embed_dim[1])
            self.patch_embed3 = OverlapPatchEmbed(
                img_size=img_size // 8, patch_size=3, stride=2, in_chans=embed_dim[1], embed_dim=embed_dim[2])
            self.patch_embed4 = OverlapPatchEmbed(
                img_size=img_size // 16, patch_size=3, stride=2, in_chans=embed_dim[2], embed_dim=embed_dim[3])
        else:
        ...
        self.blocks1 = nn.ModuleList([
            MixBlock(
                ...
                downsample=downsamples[0], conv_ffn=conv_ffn)
            for i in range(depth[0])])
        self.blocks2 = nn.ModuleList([
            MixBlock(
                dim=embed_dim[1], num_heads=num_heads[1], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                downsample=downsamples[1], conv_ffn=conv_ffn)
            for i in range(depth[1])])
        self.blocks3 = nn.ModuleList([
            TMixBlock(
                dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                downsample=downsamples[2], conv_ffn=conv_ffn)
        ...
        if self.mixture:
           self.blocks4 = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                downsample=downsamples[3], conv_ffn=conv_ffn)
...
def ds_net_small(pretrained=False, **kwargs):
    """ 20M parameters, compared with ResNet50 """
    model = MixVisionTransformer(
...
@register_model
def ds_net_big(pretrained=False, **kwargs):
    """ 20M parameters, compared with ResNet50 """
    model = MixVisionTransformer(
        patch_size=16, depth=[3, 4, 28, 3], embed_dim=[64, 128, 320, 512], 
...
@register_model
def ds_net_large(pretrained=False, **kwargs):
    """ 20M parameters, compared with ResNet50 """
    model = MixVisionTransformer(
        patch_size=16, embed_dim=[80, 160, 440, 640],
        depth=[3, 4, 28, 3], mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model
